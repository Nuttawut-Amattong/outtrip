...
function Login({ onLoginSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // สมมติ backend login ใช้ email หรือ username ใน field 'username'
      const res = await fetch("http://localhost:5000/api/auth/login", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email: user, password }), // ถ้า backend ใช้ email เปลี่ยนชื่อ property ให้ตรงกันด้วยนะ
      });
      ...
      if (res.ok) {
        setError("");
        setLoading(false);
        // เรียก callback เพื่อบอกว่าล็อกอินสำเร็จ พร้อมข้อมูล user + token
